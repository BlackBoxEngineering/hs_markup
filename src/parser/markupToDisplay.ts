...
export type MarkupNode =
  ...
  | { type: 'element'; tag: string; children: MarkupNode[] };
...
export function parseMarkup(input: string): MarkupNode[] {
  ...
  const stack: { tag: string; children: MarkupNode[] }[] = [];
  const current = () => (stack.length ? stack[stack.length - 1].children : root);

  for (let i = 0; i < tokens.length; i++) {
    const mod = i % 3;

    if (mod === 2) continue; // inner capture group artefact — skip
    ...
    if (mod === 1) {
      ...
      lines.forEach((line, idx) => {
        if (idx > 0) current().push({ type: 'text', value: '\n' });
